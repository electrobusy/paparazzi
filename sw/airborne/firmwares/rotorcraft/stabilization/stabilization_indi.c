...
static void stabilization_indi_calc_cmd(struct Int32Quat *att_err, bool rate_control, bool in_flight)
{
  ...
  if (rate_control) { //Check if we are running the rate controller
    rate_ref.p = stabilization_rate_sp.p; // (float)radio_control.values[RADIO_ROLL]  / MAX_PPRZ * STABILIZATION_INDI_MAX_RATE;
    rate_ref.q = stabilization_rate_sp.q; // (float)radio_control.values[RADIO_PITCH] / MAX_PPRZ * STABILIZATION_INDI_MAX_RATE;
    rate_ref.r = stabilization_rate_sp.r; // (float)radio_control.values[RADIO_YAW]   / MAX_PPRZ * STABILIZATION_INDI_MAX_RATE;
  } else {
